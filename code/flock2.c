///////////
// Changes made:
// 1. Added four advanced modes of different inputs
// 2. Added a hierarchical merging step based on density change between centroids of two hyper-regions
// 3. Picked the 3 most different dimensions between the population centroids to judge whether the two parts should be merged to avoid curse of dimensionality
// 4. Removed checking time parameter (by John Campbell)
...
/***************************************************************************************************************************************
	...
	Usage: flock data_file
		    Note: the input file format must be a txt file with channel values and the delimiter between two values must be a tab.
			There are additional four advanced modes:
  ...
  for (i=0;i<num_clust;i++)
    {
      for (j=0;j<num_dm;j++)
		...
		{
          population_center[i][j]=0;
		  printf("size_c[%d]=0 in ID2center\n",i);
		}
    }
...
int main (int argc, char **argv)
{
  ...
  */
  ...
  if (argc==3)
  {
	...
	printf("num of maximum pop is %d\n",max_num_pop);
  }
  ...
  if (num_bin>=1)  //num_bin has been selected by user
  {
	...
	printf("user set bin number is %d\n",num_bin); 
  }
  ...
  printf("Number of merged groups before second-run partitioning is %d\n",num_population);
  ...
  ////// there needs to be another step to further partition the data
	  ...
	  printf("Number of groups after partitioning is %d\n", num_computed_population);
...
*/
  ...
  free(center_2);
  free(center_3);
  printf("Final number of populations is %d\n",num_real_pop);
  ...
  int US = 1000000; // microseconds

  double elapsed = (double)(et.tv_sec - st.tv_sec) + (double)((et.tv_usec - st.tv_usec))/US;
  printf("flock_time,%s,%f \n",argv[1],elapsed);
