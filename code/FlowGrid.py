...
class div_bin():
	...
	def DividingOneD(self,X,d2,index=None):
		"""
		Dividing one dimensional data into equal size bin
		"""
		if index is None:
	...
	def dividing_bins(self):
		"""
		Dividing data into bins for all dimensions.
		"""
		X=self.data
		d=X.shape[1]
		new_index_dict=self.DividingOneD(X,0)
		next_d={}
		for key,value in new_index_dict.items():
			...
			for cur_d in range(1,d):
				...
				for cur_key,cur_index in temp_dict.items():
					...
					for k,i in th_index_list.items():
						if i is not None:
							new_d[cur_key+k]=i
				if cur_d!=d-1:
...
class FlowGrid():
	def __init__(self,original_data,MinDenB=3,bin_n=4,eps=1.1, MinDenC=40):
		"""
		...
		from 1 to bin_n+1-10^8(in case the value become bin_n+1).
	...
	def dividing_bins(self):
		...
		if  self.d<10 :
			...
			if np.log(self.n)/np.log(10)>6 and self.bin_n<7:
				unique_array,unique_index,counts=div_bin(X,self.bin_n).unique()	
			else:
